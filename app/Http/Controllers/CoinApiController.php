...
class CoinApiController extends Controller
{
    ...
    /**
     * Gets all coins from api and stores in the database.
     *
    ...
    /**
     * Gets all coin prices and stores them.
     *
     * @return \Illuminate\Http\Response
     */
    public function coin_price(Request $request)
    {
        try {
            $client = new \GuzzleHttp\Client(['headers' => ['X-CoinAPI-Key' => config('app.coinapi_key')]]);
            $request = $client->get(config('app.coinapi_endpoint'));
    ...
    {
        try {
            $coins = Coin::paginate($request->all());            
